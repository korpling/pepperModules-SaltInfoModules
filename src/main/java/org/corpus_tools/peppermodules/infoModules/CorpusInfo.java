...
public class CorpusInfo extends ContainerInfo implements SaltInfoDictionary {
	...
	/**
	 * Retrieves all necessary data contained in the passes {@link SCorpus}
	 * object and filles the agregator object for meta data.
	 */
	...
	/**
	 * Retrieves all necessary data contained in the passes {@link SDocument}
	 * object and filles the agregator objects of this object, like
	 * {@link #getStructuralInfo()}, {@link #getMetaDataInfo()} and
	 * {@link #getAnnotations()}.
	 */
			...
			if (cont.getAnnotations() != null) {
				for (String sLayerName : cont.getAnnotations().keySet()) {
					...
					if (!getAnnotations().containsKey(sLayerName)) {
						// when this annotations does not contain such a layer,
						// copy
						...
						for (String sAnnoName : anno.keySet()) {
							if (!anno_self.containsKey(sAnnoName)) {
								...
								for (String SAnnoValue : anno.get(sAnnoName).keySet()) {
									// add as much values have been in given
									// anno table
