...
A more detailed description of the visualization, the customization and the outputs can be found in section [HTML output](#htmlOut) and [XML output](#xmlOut).
...
The SaltInfoExporter comes with a default flat design style. For our demonstration we used the pcc2 corpus, which is a subset of the pcc corpus (see: https://www.ling.uni-potsdam.de/acl-lab/Forsch/pcc/pcc.html). The corpus site generated by the SaltInfoExporter of this corpus can be found [here](http://korpling.github.io/pepperModules-SaltInfoModules/samples/pcc2/).

![sample html output for pcc corpus](./gh-site/img/sample_pcc_all.png)

### Name and description
On top of the site you see the name of the corpus which is 'pcc2' in our case and a short text, which could be the full name of the corpus or even a small description. 

### Navigation
On the left you see the navigation bar, which displays a tree structure containing the root corpus, sub corpora and even all deocuments belonging to the corpus. In our sample the root corpus is the 'pcc2' corpus which consists of the two documents '11299' and '4282'. By clicking on the small triangle next to a corpus or sub corpus name, you can collapse or expand the sub corpus structure.
...
To customize the style of the corpus site, you can adapt the CSS file named 
```
style.css
```
...
customization.json
```
which is to find in the root folder of the export. As the file ending already indicates, this file is a JSON file following the JSON syntax (see: http://json.org/). For most entries in that file, you just need to determine attribute-value-pairs whose name and value is embraced by quotation marks. Name and value are separated by a colon. To give an example the following snippet shows the definition of the little text which is displayed beyond the name of the corpus:
```json
{
    ...
...
To adapt the description of the corpus, change the entry for the variable "description". To change the author names and mail addresses, change the entry for "annotators" as shown with the following snippet:
...
Each section like the structural info section, the meta data section, the annotation section and even the sections for each layer can have a description. A description is given per default, but this might not be an adequate one. To adapt the description, change the following entries in the json file:
```json
...
The next snippet shows the adaption of the json file for descriptions to the single layers. The used layers come from the pcc2 corpus and are 'exmaralda' and 'mmax'.
```json
    "layerDesc" : [
...
```
#### ANNIS links
When your corpus is also available in a public accessible ANNIS instance you can add a link to the instance in the JSON file. This allows the corpus site to create links directing to the ANNIS instance. The corpus site then enables three different links to ANNIS:

1. a link in the header section of the site to the corpus in the ANNIS instance
1. a link in each annotation name to the ANNIS instance querying the annotation name
1. a link in each annotation value to the ANNIS instance querying the annotation name and value
...
#### Theme
The corpus site can be produces in different themes. To customize the theme you need to set a customization property, which influences the creation of the site. More information on changing the site's theme can be found <a href="#sec_theme">here</a>. A sample of the historic scheme is the [Tatian corpus site](http://korpling.github.io/pepperModules-SaltInfoModules/samples/b4.tatian/) of the B4 project of the [Sonderforschungsbereich 632](https://www.sfb632.uni-potsdam.de/en/).
...
### <a name="sec_htmlInterpreted">htmlInterpreted</a>
Determines whether the short description in the html output will be interpreted as html code. The default is 'false'.
To use the html interpreted set use the property in the workflow description as follows:
```xml
